...
def compute_trajectory_stats(traj_info):
    ...
    # Calculate time for generated steps (excluding oracle infill time)
    time_for_generated = 0.0
    if parent_id is None:
        # Root: sum all policy times
        time_for_generated = sum(t.metrics.get('policy_time', 0) for t in transitions)
    else:
        # Branched (oracle): sum only policy times for newly generated steps
        # First transition might have oracle infill (policy_time=0), rest are student
        parent_transition_idx = branch_info["parent_transition_idx"]
...
async def main(
    ...
    log_dir: str = "logs/prefix_testing",
):
    ...
    if len(all_trajectory_info) > 1:
        ...
        # Save plot
        plot_path = os.path.join(log_dir, "tree_visualization_oracle.png")
        ...
        # Save plot
        timeline_plot_path = os.path.join(log_dir, "trajectory_completion_timeline_oracle.png")
        ...
        csv_path = os.path.join(log_dir, "trajectory_stats_oracle.csv")
