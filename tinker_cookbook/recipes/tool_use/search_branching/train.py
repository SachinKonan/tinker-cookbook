"""
CLI for Search-R1 with tree-based branching
"""
...
from tinker_cookbook.recipes.tool_use.search_branching.search_env import SearchBranchingDatasetBuilder
from tinker_cookbook.recipes.tool_use.search_branching.tools import (
...
async def cli_main(cli_config: CLIConfig):
    ...
    run_name = (
        f"search_branching_{model_name_short}_{bs_str}_gs{cli_config.group_size}"
        f"_src{cli_config.src_trajectories}_branches{cli_config.num_branches}"
        f"_seed{cli_config.seed}_tracj{cli_config.max_trajectory_tokens // 1024}k"
    ...
    if cli_config.log_path is not None:
        ...
        log_path = f"/tmp/tinker-examples/rl_search_branching/{run_name}"
