...
class SearchBranchingDataset(SearchR1Dataset):
    """Dataset for tree-based branching that creates fewer initial environments."""

    def __init__(
        self,
        batch_size: int,
        group_size: int,
        src_trajectories: int,
        renderer: renderers.Renderer,
        gaia_tool_client: GAIAToolClient,
        ...
        self.src_trajectories = src_trajectories

    def _make_env_group_builder(self, row: SearchR1Datum, group_size: int) -> ProblemGroupBuilder:
        """Create env group builder with src_trajectories envs instead of group_size."""
        return ProblemGroupBuilder(
            env_thunk=partial(
                SearchEnv,
                row["question"],
            ...
            num_envs=self.src_trajectories,  # Use src_trajectories instead of group_size
        )


@chz.chz
class SearchBranchingDatasetBuilder(RLDatasetBuilder):
    """Dataset builder for tree-based branching training with GAIA tools.

    Creates dataset with src_trajectories initial environments per group,
    which will be branched to reach group_size total trajectories during training.
    """

    batch_size: int
    group_size: int  # Target size after branching
    src_trajectories: int  # Number of initial source trajectories
    model_name_for_tokenizer: str
    renderer_name: str
    max_search_results: int = 5  # For GAIAToolClient
    convo_prefix: list[renderers.Message] | None | Literal["standard"] = "standard"
    ...
    async def __call__(self) -> tuple[SearchBranchingDataset, None]:
        if self.convo_prefix == "standard":
            convo_prefix = SearchEnv.standard_fewshot_prefix()
        else:
            convo_prefix = self.convo_prefix
        ...
        train_dataset = SearchBranchingDataset(
            batch_size=self.batch_size,
            group_size=self.group_size,
            src_trajectories=self.src_trajectories,
            renderer=renderer,
            gaia_tool_client=gaia_tool_client,
            convo_prefix=convo_prefix,
            split="train",
