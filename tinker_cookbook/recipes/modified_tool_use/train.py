"""
CLI for Modified Tool Use Training (using GAIA tools)
"""
import asyncio
import logging
...
from tinker_cookbook.recipes.modified_tool_use.modified_search_env import SearchR1DatasetBuilder
...
async def cli_main(cli_config: CLIConfig):
    ...
    run_name = f"modified_tool_use_{model_name_short}_{bs_str}_gs{cli_config.group_size}_seed{cli_config.seed}_traj{cli_config.max_trajectory_tokens // 1024}k_lr{cli_config.learning_rate}_rank{cli_config.lora_rank}_{date_and_time}"

    # Set log path
    if cli_config.log_path is not None:
        log_path = cli_config.log_path
    else:
        log_path = f"/tmp/tinker-examples/modified_tool_use/{run_name}"
