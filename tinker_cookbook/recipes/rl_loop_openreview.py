...
class Config:
    ...
    log_path: str = "/tmp/tinker-examples/rl-loop-openreview"
...
def main(config: Config):
    ...
    train_dataset, _ = load_openreview_data(config.data_path, config.dry_run)
    ...
    for batch_idx in range(start_batch, n_train_batches):
        ...
        for sample_futures, prompt_tokens, sample in zip(
            batch_futures, batch_prompts, batch_samples
        ):
            gt_rating = sample['rating']
            ...
            for future in sample_futures:
                ...
                reward, _ = parse_response_and_get_reward(parsed_message["content"], gt_rating)
        ...
        if batch_idx % 10 == 0:
            logger.info(
                ...
                f"Mean reward: {metrics['reward/mean']:.4f} | "
                f"Time: {metrics['time/total']:.2f}s"
            )
